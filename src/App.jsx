...
const NAMA_KAFE = "AY Cafe";
const ALAMAT_KAFE = "Jl. Contoh No. 123, Jakarta";

function Struk({ order, nomorAntrian }) {
  // PENTING: hook ini harus selalu dipanggil di posisi yang sama setiap
  // render, jadi ditempatkan SEBELUM early return "if (!order)" di bawah —
  // menempatkannya setelah early return akan melanggar Rules of Hooks dan
  // menyebabkan React error #310 (sama seperti kasus PosApp sebelumnya).
  const itemsUntukStruk = useMemo(() => {
    if (!order) return [];
    // Struk untuk PELANGGAN tidak perlu menunjukkan entry-entry terpisah yang
    // dipakai internal untuk melacak status dapur (lihat tambahItem) — di sini
    // semua entry dengan menu & harga yang sama digabung jadi satu baris, supaya
    // misalnya "Nasi Goreng 1x + Nasi Goreng 1x" tampil rapi sebagai "Nasi Goreng 2x".
    const map = {};
    order.items.forEach((i) => {
      const key = `${i.menuId}-${i.harga}`;
  ...
  return (
    <div className="struk-cetak" id="area-struk">
      ...
      <div className="struk-garis" />
      {itemsUntukStruk.map((i, idx) => (
        <div className="struk-item" key={i.id || `${i.menuId}-${idx}`}>
          <div className="struk-baris">
            <span>{i.nama}</span>
          </div>
          <div className="struk-baris">
...
function KasirView({
  ...
  const tambahItem = async (item) => {
    ...
    // Cari entry yang sama DAN masih berstatus "menunggu" — qty boleh
    // digabung ke entry ini karena belum ada yang mulai dikerjakan dapur.
    const idx = items.findIndex(
      (i) => i.menuId === item.id && (i.statusDapur || "menunggu") === "menunggu"
    );
    if (idx >= 0) {
      items[idx] = { ...items[idx], qty: items[idx].qty + 1 };
    } else {
      // Tidak ada entry yang masih "menunggu" untuk menu ini — entah karena
      // belum pernah dipesan sama sekali, atau entry yang ada sudah mulai
      // diproses/sudah siap di dapur. Pada kedua kasus, tambahan ini HARUS
      // jadi baris baru dengan status "menunggu" sendiri, supaya progres
      // dapur untuk porsi yang sudah dikerjakan tidak ikut ter-reset.
      items.push({
        id: uid(), // id unik PER ENTRY (beda dari menuId) — penting karena
                   // sekarang satu menuId bisa punya lebih dari satu entry
                   // dalam satu order (lihat komentar di atas)
        menuId: item.id,
        nama: item.nama,
        harga: item.harga,
  ...
  const ubahQty = async (itemId, delta) => {
    if (!activeOrder) return;
    let items = activeOrder.items.map((i) =>
      i.id === itemId ? { ...i, qty: i.qty + delta } : i
    );
  ...
  const hapusItem = async (itemId) => {
    if (!activeOrder) return;
    const items = activeOrder.items.filter((i) => i.id !== itemId);
    const updated = { ...activeOrder, items };
    await persistOrders({ ...orders, [updated.id]: updated });
  };
          ...
          {activeOrder?.items.map((i) => (
            <div className="order-item" key={i.id}>
              ...
              <div className="qty-ctl">
                <button onClick={() => ubahQty(i.id, -1)}><Minus size={14} /></button>
                <span>{i.qty}</span>
                <button onClick={() => ubahQty(i.id, 1)}><Plus size={14} /></button>
              </div>
              <span className="order-item-subtotal">{formatRupiah(i.harga * i.qty)}</span>
              <button className="icon-btn-danger" onClick={() => hapusItem(i.id)}>
                <Trash2 size={15} />
              </button>
            </div>
          ))}
...
function DapurView({ orders, meja, persistOrders, persistMeja }) {
  ...
  const ubahStatusItem = async (orderId, itemId, statusBaru) => {
    const ord = orders[orderId];
    if (!ord) return;
    // Cocokkan berdasarkan id unik per entry. Fallback ke menuId HANYA untuk
    // data lama (order yang dibuat sebelum field id ditambahkan) yang tidak
    // punya id sama sekali — pada data baru, id selalu ada dan unik per entry.
    const items = ord.items.map((i) => {
      const cocok = itemId.startsWith("legacy:")
        ? i.menuId === itemId.slice(7) && !i.id
        : i.id === itemId;
      return cocok ? { ...i, statusDapur: statusBaru } : i;
    });
    const updated = { ...ord, items };
    await persistOrders({ ...orders, [updated.id]: updated });
              ...
              >
                ...
                <div className="dapur-items">
                  {o.items.map((item, idx) => {
                    ...
                    const itemKey = item.id || `legacy:${item.menuId}`;
                    return (
                      <button
                        key={item.id || `${item.menuId}-${idx}`}
                        className={"dapur-item dapur-item-" + status}
                        onClick={() => ubahStatusItem(o.id, itemKey, siklusStatus(status))}
                      >
                ...
                <>
                  <div className="detail-meja-items">
                    {detailOrder.items.map((i, idx) => (
                      <div className="detail-meja-item" key={i.id || `${i.menuId}-${idx}`}>
                        <span>
                          {i.qty}× {i.nama}
